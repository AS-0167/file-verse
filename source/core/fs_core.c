...
int fs_init(void** instance, const char* omni_path, const char* config_path) {
    ...
    if (!node_map) { free(entries); return OFS_ERROR_SYSTEM; }

    FileEntry root_entry = entries[1];
    ofs->file_tree->root->entry_index = root_entry.entry_index;
    ofs->file_tree->root->owner_id = root_entry.owner_id;
    ofs->file_tree->root->permissions = root_entry.permissions;
    ofs->file_tree->root->created_time = root_entry.created_time;
    ofs->file_tree->root->modified_time = root_entry.modified_time;
    ...
    for (uint32_t i = 2; i < ofs->header.max_files; i++) {
        if (entries[i].is_valid == 0) {
            ...
            new_node->owner_id = entries[i].owner_id;
            new_node->permissions = entries[i].permissions;
            new_node->size = entries[i].total_size;
            new_node->start_block = entries[i].start_block;
            new_node->created_time = entries[i].created_time;
            new_node->modified_time = entries[i].modified_time;
    ...
    for (uint32_t i = 2; i < ofs->header.max_files; i++) {
        if (node_map[i] != NULL) {
            ...
            if (parent_idx == 0 || parent_idx >= ofs->header.max_files || !node_map[parent_idx]) {
                printf("WARNING: Entry %u ('%s') has invalid parent index %u. Skipping.\n", i, node_map[i]->name, parent_idx);
                fsnode_destroy(node_map[i]);
                node_map[i] = NULL;
                continue;
            }
            ...
            char* parent_path = fstree_get_path(parent_node);
            ...
            if (parent_path && strcmp(parent_path, "/") == 0) {
                ...
                snprintf(child_path, sizeof(child_path), "%s/%s", parent_path ? parent_path : "", node_map[i]->name);
            }
...
static void free_generic_ptr(void* value) {
    if (value) free(value);
}
...
void fs_shutdown(void* instance) {
    ...
    if (ofs->users) ht_destroy(ofs->users, free_generic_ptr);
    if (ofs->file_tree) fstree_destroy(ofs->file_tree);
    if (ofs->free_blocks) bitmap_destroy(ofs->free_blocks);
    if (ofs->sessions) ht_destroy(ofs->sessions, free_generic_ptr);
